...
def get_alloc():
    # TOPE de BTC a usar al comprar. Si existe alloc_<SYMBOL>.txt, usa ESE numero.
    # Asi agregas o sacas plata sin tocar el codigo: echo "0.03" > el archivo, y listo.
    try:
...
def do_buy(bal_before):
    ...
    alloc = get_alloc()
    usable = min(btc, alloc)                    # NUNCA gasto mas que mi tope (deja BTC al otro bot)
    quote = math.floor(usable * BUY_BUFFER * 1e8) / 1e8
    if quote < FILT["minNotional"]:
        log(f"BTC insuficiente para comprar (tengo {btc}, tope {alloc}). Lo trato como ya-FLAT.")
        return
    if btc > alloc:
        log(f"tope ALLOC={alloc:.8f} BTC -> uso {usable:.8f} de {btc:.8f} disponible")
...
def main():
    ...
    log(f"=== bot_v2 arrancado | DRY_RUN={DRY_RUN} | {SYMBOL} {INTERVAL} | "
        f"COMBO SAR({AF_STEP},{AF_MAX})+ST({ST_ATR_LEN},{ST_MULT}) | tope={get_alloc():.8f}BTC | "
